...
/** \brief Retorna el total de ventas que cumplen con la función criterio
 * \param this LinkedList* Puntero a la lista
 * \param fnCriterio (*fnCriterio) Puntero a la funcion criterio
 * \return Si pudo llevar a cabo la operación, denota la cantidad de ventas; caso contrario denota -1.
 */

 int ll_count(LinkedList* this, int (*fnCriterio)(void*, int, int), int montoMayor)
 {
    int ret = -1;
    Node* pNode;
    int i;
    ...
    if(this != NULL && fnCriterio != NULL)
    {
        ...
        if(pNode != NULL)
        {
            for(i=0; i<length; i++)
            {
                contador = contador + fnCriterio(this, i, montoMayor);
                pNode = pNode->pNextNode;
            }
        }
        ret = contador;
